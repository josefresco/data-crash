extends TestCase
## The four remaining bosses: Crapya Butella (Phase 2 automated defenses),
## Sham Crapman (wave 2), Fark Suckerbush (wave 4), Harry Perckerson (wave 5).
##
...
func _run() -> void:
	...
	await seconds(0.5)
	for name in ["Guard1", "Guard2", "Guard3", "Dog1", "Dog2", "PatrolFelsa", "SupplyVan", "StrayDog1", "StrayDog2"]:
...
func _test_crapya() -> void:
	...
	var unit := get_tree().get_nodes_in_group("cooling_units")[0] as Destructible
	...
	for name in ["SteamVent1", "RackCrusher1"]:
		var trap := level.get_node(name) as SteamVent
	...
	check((level.get_node("SteamVent1") as SteamVent).state == SteamVent.State.OFF, "steam vents shut down")
...
func _test_boss_waves() -> void:
	var spawner := level.get_node("WaveSpawner") as WaveSpawner
	check(spawner.waves[1].has("sham") and spawner.waves[3].has("fark") and spawner.waves[4].has("harry"),
		"bosses lead waves 2, 4, and 5")
